...
*/
/**
 * Parole review detail screen behavior.
 * 
 * Author: Josh Divine
...
window.onload = function() {	
	...
	applyActionMenu(document.getElementById("paroleReviewNotesActionMenuLink"), paroleReviewNoteActionMenuOnClick);
	for (var index = 0; index < currentParoleReviewNoteIndex; index++) {
		paroleReviewNoteRowOnClick(index);
	}
	paroleReviewDocumentAssociationItemsCreateOnClick();
	for(var i = 0; i < currentDocumentAssociationItemIndex; i++){
		paroleReviewDocumentAssociationItemRowOnClick(i);
		for(var j = 0; j < currentDocumentTagItemIndexes[i]; j++){
			documentTagItemRowOnClick(i,j);
		}
	}
	applyFormUpdateChecker(document.getElementById("paroleReviewForm"));
};