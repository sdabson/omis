<%--
  - Offender contact telephone number edit table row.
  -
  - Author: Josh Divine
  --%>
...
<c:if test="${not empty telephoneNumberItem.operation}">
	...
	<tr id="telephoneNumberItems[${telephoneNumberItemIndex}].row" class="${rowClass}">
		<td>
			<a id="telephoneNumberItems[${telephoneNumberItemIndex}].removeLink" class="removeLink" href="${pageContext.request.contextPath}/offenderContact/contact/removeTelephoneNumber.html?telephoneNumber=${telephoneNumberItem.telephoneNumber.id}" title="<fmt:message key='removeLink' bundle='${commonBundle}'/>"><span class="linkLabel"><fmt:message key="removeLink" bundle="${commonBundle}"/></span></a>
