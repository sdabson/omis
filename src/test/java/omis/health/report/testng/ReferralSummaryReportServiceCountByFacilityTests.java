...
/**
 * Test the counting referral summary by facility
 *
 * @author Yidong Li
...
public class ReferralSummaryReportServiceCountByFacilityTests 
	...
	public void testCountingReferralSummaryByFacility()
		...
		City secondBirthPlace = this.cityDelegate.findOrCreate("Butt",
			true, birthState, birthCountry);
		Offender secondOffender = this.offenderDelegate.create("djyt", 
			"ejkyt", "dht", "Ms.", 987654321, "34",
			this.parseDateText("12/12/2017"), birthCountry,
			birthState, secondBirthPlace, Sex.FEMALE);
		OffenderAppointmentAssociation secondOffenderAppointmentAssociation
		= this.offenderAppointmentAssociationDelegate.create(
			secondOffender, date, facility, startTime, endTime);
		...
		= this.externalReferralAuthorizationRequestDelegate.create(
			secondOffender, null, null, medicalFacility, facility,
		...
		// Action
		long count = this.referralSummaryReportService.countByFacility(
			facility, null, null, types, statuses);
