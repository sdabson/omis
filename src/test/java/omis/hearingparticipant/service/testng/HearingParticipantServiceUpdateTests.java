...
public class HearingParticipantServiceUpdateTests
	...
	/**
	 * Tests updating a Hearing Participant.
	 * @throws DuplicateEntityFoundException - When a duplicate entity it found
	 */
	@Test
	public void testHearingParticipantUpdate()
			throws DuplicateEntityFoundException {
		final BoardHearing boardHearing = this.createBoardHearing();
		final Person oldPerson = this.personDelegate.create("Teaville", "Chai",
				null, null);
		HearingParticipant hearingParticipant = this.hearingParticipantDelegate
				.create(boardHearing, oldPerson,
						HearingParticipantCategory.STAFF,
		...
		hearingParticipant = this.hearingParticipantService
				.updateHearingParticipant(
						hearingParticipant, person, category, boardApproved,
						witness, facilityApproved, intent, comments);
	...
	/**
	 * Tests Updating a Hearing Participant Note.
	 * @throws DuplicateEntityFoundException - When a duplicate entity it found
	 */
	@Test
	public void testHearingParticipantNoteUpdate()
			throws DuplicateEntityFoundException {
		final BoardHearing boardHearing = this.createBoardHearing();
		final Person person = this.personDelegate.create("McBeans", "Coffee",
				"Java", null);
		...
		HearingParticipantNote hearingParticipantNote =
				this.hearingParticipantNoteDelegate.create(hearingParticipant,
						"Old Description", this.parseDateText("12/12/2017"));
		final String description = "Note Description";
		final Date date = this.parseDateText("01/01/2018");
		
		hearingParticipantNote =
				this.hearingParticipantService.updateHearingParticipantNote(
						hearingParticipantNote, hearingParticipant,
						description, date);
