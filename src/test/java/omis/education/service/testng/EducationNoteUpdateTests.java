...
public class EducationNoteUpdateTests 
	...
	public void testUpdate()
		...
		EducationNote note = this.educationService.createNote(
				educationTerm, this.parseDateText("01/01/2001"), 
				"firstNoteDesc");
		
		
		final Date noteDate = this.parseDateText("07/07/2016");
		final String noteDescription = "Note Description";
		note = this.educationService.updateNote(note, noteDate, 
					noteDescription);
		

		//educationNote.getEducationTerm() to educationTerm
		assert educationTerm.equals(note.getEducationTerm())
		: String.format("Wrong education term for the note: "
				+ "#%d found; #%d expected",
				note.getEducationTerm().getId(), 
				educationTerm.getId());
		//educationNote.getDate() to noteDate
		assert noteDate.equals(note.getDate())
		: String.format("Wrong note date: %s found; %s expected",
				note.getDate(), 
				noteDate);
		//educationNote.getDescription() to noteDescription
		assert noteDescription.equals(note.getDescription())
		: String.format("Wrong note description: %s found; %s expected",
				note.getDescription(), 
				noteDescription);
	}
