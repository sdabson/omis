...
/**
 * EducationalAchievementUpdateTests.java
 * 
...
public class EducationalAchievementUpdateTests 
	...
	public void testUpdate()
		...
		final EducationTerm educationTerm
			= this.educationService.createEducationTermWithoutAchievement(
					offender, description, specialEducation, attendedDateRange, 
					institute, verificationSignature);
		
		EducationalAchievement achievement 
		= this.educationService.createEducationalAchievement(educationTerm, 
				this.parseDateText("10/10/2010"), "firstAcheDesc", 
				this.achievementCategoryDelegate.create("firstAcheCat", 
						(short) 2, true));
		...
		achievement = this.educationService.createEducationalAchievement(
				educationTerm, achievementDate, achievementDescription, 
				acheCat);
		

		//achievement.getEducationTerm() to educationTerm
		assert educationTerm.equals(achievement.getEducationTerm())
		: String.format("Wrong education term for the achievement: "
				+ "#%d found; #S%d expected",
				achievement.getEducationTerm().getId(), 
				educationTerm.getId());
		//achievement.getDate() to achievementDate()
		assert educationTerm.equals(achievement.getEducationTerm())
		: String.format("Wrong education term for the achievement: "
				+ "%s found; %s expected",
				achievement.getEducationTerm().getDescription(), 
				educationTerm.getDescription());
		//achievement.getDescription() to achievementDescription
		assert achievementDescription.equals(achievement.getDescription())
		: String.format("Wrong achievement description: %s found; %s expected",
				achievement.getDescription(), 
				achievementDescription);
		//achievement.getCategory() to achievementCategory
		assert acheCat.equals(achievement.getCategory())
		: String.format("Wrong achievement category: %s found; %s expected",
				achievement.getCategory().getName(), 
				acheCat.getName());
	}
