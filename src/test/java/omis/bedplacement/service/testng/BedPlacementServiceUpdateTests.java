 ...
 * @since OMIS 3.0
 */
public class BedPlacementServiceUpdateTests
	...
	/**
	 * Tests the update of the bed for a bed placement.
	 * 
	 ...
	 */
	@Test
	public void testUpdateBed() throws DuplicateEntityFoundException, 
		...
		Bed newBed = this.bedDelegate.create(room, 2, true);

		// Action
		bedPlacement = this.bedPlacementService.update(bedPlacement, newBed, 
				confirmed, dateRange, description, reason);

		// Assertions
		PropertyValueAsserter.create()
			.addExpectedValue("offender", offender)
			.addExpectedValue("bed", newBed)
	...
	/**
	 * Tests the update of the confirmed status for a bed placement.
	 * 
	 * @throws BedPlacementDateConflictException if dates conflict
	 * @throws BedOccupiedException if the bed already has an occupant
	 * @throws DuplicateEntityFoundException if the entity already exists
	 */
	@Test
	public void testUpdateConfirmed() throws DuplicateEntityFoundException, 
			BedPlacementDateConflictException, BedOccupiedException {
		// Arrangements
		Offender offender = this.offenderDelegate.createWithoutIdentity("Smith", 
				"John", "Jay", null);
		...
		// Action
		bedPlacement = this.bedPlacementService.update(bedPlacement, bed, 
				newConfirmed, dateRange, description, reason);
	...
	/**
	 * Tests the update of the date range for a bed placement.
	 * 
	 * @throws BedPlacementDateConflictException if dates conflict
	 * @throws BedOccupiedException if the bed already has an occupant
	 * @throws DuplicateEntityFoundException if the entity already exists
	...
	public void testUpdateDateRange() throws DuplicateEntityFoundException, 
		...
		BedPlacement bedPlacement = this.bedPlacementDelegate.create(offender, 
				bed, confirmed, dateRange, description, reason);
		DateRange newDateRange = new DateRange(this.parseDateText("01/01/2017"), 
				this.parseDateText("06/01/2017"));

		// Action
		bedPlacement = this.bedPlacementService.update(bedPlacement, bed, 
				confirmed, newDateRange, description, reason);
	...
	/**
	 * Tests the update of the description for a bed placement.
	 * 
	 * @throws BedPlacementDateConflictException if dates conflict
	 * @throws BedOccupiedException if the bed already has an occupant
	 * @throws DuplicateEntityFoundException if the entity already exists
	 */
	@Test
	public void testUpdateDescription() throws DuplicateEntityFoundException, 
			BedPlacementDateConflictException, BedOccupiedException {
		// Arrangements
		Offender offender = this.offenderDelegate.createWithoutIdentity("Smith", 
				"John", "Jay", null);
		...
		// Action
		bedPlacement = this.bedPlacementService.update(bedPlacement, bed, 
				confirmed, dateRange, newDescription, reason);
	...
	/**
	 * Tests the update of the reason for a bed placement.
	 * 
	 * @throws BedPlacementDateConflictException if dates conflict
	 * @throws BedOccupiedException if the bed already has an occupant
	 * @throws DuplicateEntityFoundException if the entity already exists
	 */
	@Test
	public void testUpdateReason() throws DuplicateEntityFoundException, 
			BedPlacementDateConflictException, BedOccupiedException {
		// Arrangements
		Offender offender = this.offenderDelegate.createWithoutIdentity("Smith", 
				"John", "Jay", null);
		...
		BedPlacement bedPlacement = this.bedPlacementDelegate.create(offender, 
				bed, confirmed, dateRange, description, reason);
		BedPlacementReason newReason = this.bedPlacementReasonDelegate
				.create("Reason 2", 2, true);

		// Action
		bedPlacement = this.bedPlacementService.update(bedPlacement, bed, 
				confirmed, dateRange, description, newReason);
		...
		PropertyValueAsserter.create()
			...
			.addExpectedValue("dateRange", dateRange)
			.addExpectedValue("description", description)
			.addExpectedValue("bedPlacementReason", newReason)
			.performAssertions(bedPlacement);
	}
	...
	public void testDuplicateEntityFoundException() 
		...
		DateRange secondDateRange = new DateRange(
				...
				this.parseDateText("06/01/2017"));
		BedPlacement bedPlacement = this.bedPlacementDelegate.create(offender, 
				bed, confirmed, secondDateRange, description, reason);
	...
	public void testBedPlacementDateConflictException() 
		...
		DateRange overlappingDateRange = new DateRange(
				this.parseDateText("02/01/2017"), null);
		
		// Action
		bedPlacement = this.bedPlacementService.update(bedPlacement, bed, 
				confirmed, overlappingDateRange, description, reason);
	}
